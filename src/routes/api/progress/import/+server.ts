...
import { parseAttempt, parseCardEntry, parseDiscovery, parseList, requireRecord } from '$lib/server/validate';
...
/**
 * Body `{ attempts: Attempt[], cards: { bundleId, epd, state }[], discoveries?: Discovery[] }` — a signed-out learner's
...
export const POST: RequestHandler = authed(async ({ request }, { user, db }) => {
	...
	const result = await importProgress(db, user.id, { attempts, cards, discoveries }, new Date());
