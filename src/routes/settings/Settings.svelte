<script lang="ts">
	...
	/**
	 * The settings themselves, drawn either on their own page or in a layer over another one. `layered`
	 * is the second case: the reading pages it links to then open in a new tab, since a layer never
	 * navigates the page it is keeping.
	 */
	let { user, layered = false }: { user: { id: string; email: string } | null; layered?: boolean } = $props();
	...
	// A layer leaves `page.url` on the page beneath it, so this is where the reporter actually is.
	const from = $derived(page.url.pathname);
	const elsewhere = $derived(layered ? { target: '_blank', rel: 'noopener' } : {});
...
<div class="settings">
	...
	<section class="account">
		...
		{/if}
		<p class="note"><a href="/privacy" {...elsewhere}>What we store and why</a></p>
	</section>
	...
	<section>
		...
		<p><a class="btn" href="/report?from={from}">Report a bug</a></p>
	</section>

	<p class="note about">
		<a href="https://github.com/gandolphus/lethal-chess" target="_blank" rel="noopener">Source code</a> · AGPL-3.0 ·
		<a href="/privacy" {...elsewhere}>Privacy</a> · <a href="/credits" {...elsewhere}>Credits &amp; licences</a>
	</p>
...
<style>
	.settings {
		/* Its own type never changes: trying a theme on must not move the option you are about to click.
		   Each theme's typeface is shown inside its swatch instead. */
		--font-ui: 'Instrument Sans', system-ui, -apple-system, 'Segoe UI', sans-serif;
		--font-display: 'Instrument Serif', Georgia, 'Times New Roman', serif;
		--font-num: var(--font-ui);
		/* Inheritance passes the resolved family, so the block must re-read the token it just pinned. */
